...
interface MovimentacaoFormProps {
  ...
  onCancel: () => void
  lotes: Array<{ id: string; codigo: string; fornecedorNome: string; quantidadeDisponivel: number }>
  tipo?: 'entrada' | 'saida' | 'transferencia' | 'ajuste'
    ...
    <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
                ...
                {lotes.map((lote) => (
                  <SelectItem key={lote.id} value={lote.id}>
                    <div className="flex items-center justify-between w-full">
                      <span>{lote.codigo} - {lote.fornecedorNome}</span>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              {errors.dataMovimentacao && (
                <p className="text-sm text-red-600">{errors.dataMovimentacao.message}</p>
              )}
            </div>
          </div>
        </CardContent>
      ...
      <div className="flex justify-end space-x-2">
        ...
        <Button type="submit" disabled={isSubmitting}>
          {isSubmitting ? 'Salvando...' : movimentacao ? 'Atualizar' : 'Registrar Movimentação'}
        </Button>
      </div>
    </form>
  )
